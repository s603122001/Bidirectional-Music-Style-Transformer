...
def midi2score(song,
    ...
    score = np.zeros((int(length / sec_per_tick) + 1, 88))

    for msg in mid:
        time += msg.time
        pos = int(np.round(time / sec_per_tick))
        if (pos + 1 > score.shape[0]):
            score = np.append(score, np.zeros((pos - score.shape[0] + 1, 88)), axis=0)
        if (msg.is_meta):
            if (msg.type == 'set_tempo'):
...
def score2midi(name,
    ...
    pitch_table = np.zeros((len(pitch_range), 1))
    ...
    for index, tick in enumerate(score):
        ...
        for _note in current_notes:
            if (tick[_note] == 0):
                track.append(Message('note_on', note=_note + pitch_range[0], velocity=0, time=0))
                pitch_table[_note] = 0
            ...
            if (melody_constraint == True):
                if (melody[index][_note] == 1):
                    ...
                    if (pitch_table[_note] != 2):
                        track.append(Message('note_on', note=_note + pitch_range[0], velocity=85, time=0))
                        pitch_table[_note] = 2
                        
                elif (pitch_table[_note] == 0):
                    track.append(Message('note_on', note=_note + pitch_range[0], velocity=80, time=0))
                    pitch_table[_note] = 1
            else:
